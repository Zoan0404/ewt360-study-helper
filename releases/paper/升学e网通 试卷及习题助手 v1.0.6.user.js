...
// @description  采用志成🍥 的答案获取与风月同天🌸 的脚本参考及UI页面，由⚡Zoan进行整合并支持子母题型 混合题型 并且提供扫描功能，支持检测完成状态，完成禁止再次刷，优化提交功能，进一步优化速度；修复刷题时使用试卷自身bizCode避免reportId为null的问题；修复答案获取与提交共锁同一报告导致全空0分的问题（答案获取改用已完成报告，提交报告不再提前锁定）。v1.0.4：集成官方「重新作答」接口 reAnswerPaper（204课后习题重刷后任务入口自动绑定最新报告，实测有效）；205试卷服务器拒绝重做（7771522），任务绑定固化无法改写，强制重刷仅生成满分报告并提示；锁卷取答案改为专用 homeworkId=0 报告，杜绝污染任务成绩。v1.0.5：浏览页(bizCode=201)自动探测真实作答类型（204习题/205试卷），失败回退201（VIEW浏览码）。v1.0.6：新增 webreport 报告接口全量答案获取（1 次 GET 替代逐题 analysis，含解析/子题/试题名称/卷面题数），失败自动回退逐题方案。
// @license      ISC
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 试卷及习题助手 <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.6</span></div>
          ...
          <div class="eph-about-section">
                      <div class="eph-about-section">
            <h3>✨ v1.0.6 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 新增 webreport 报告接口全量答案获取：1 次请求替代逐题 analysis（含解析/子题），失败自动回退</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 附带试题名称与卷面题数，作为 getPaperInfo 失败时的兜底数据源</div>
          </div>
<h3>✨ v1.0.5 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 浏览页(bizCode=201)自动探测真实作答类型（204习题/205试卷），失败回退201</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 类型标签三态显示：习题(204)/试卷(205)/浏览(201)</div>
          </div>
