...
// @description  采用志成🍥 的答案获取与风月同天🌸 的脚本参考及UI页面,由⚡Zoan进行整合并支持子母题型 混合题型 并且提供扫描功能,支持检测完成状态,优化提交功能,支持查看答案,进一步优化速度 v1.0.8：锁卷安全机制升级——提交答案优先使用任务已绑定报告（原报告答题），锁卷报告仅作答案源；锁卷前归属检测，部分试卷服务器忽略homeworkId=0时自动放弃锁卷，杜绝生成真实作答报告污染成绩。
// @license      ISC
...
(function() {
    ...
    function getCurrentPageTask() {
        ...
        // v1.0.8fix：URL 上的 bizCode=201（VIEW 浏览码）不是真实作答类型，
    ...
    const cleanHtmlKeepImg = (text) => {
        if (!text) return '';
        // v1.0.8fix：公式/图片 http → https（https 页面加载 http 混合内容会被浏览器拦截，导致公式图显示为空）
    ...
    // v1.0.8fix：模块级缓存——刷一套卷 userId 只请求 1 次（webreport / getQuestions 降级共用），不再重复 3-5 次
    ...
    // ---------- v1.0.8fix：webreport 全量答案接口（1 次 GET 拿全卷答案+解析，替代逐题 analysis） ----------
    // 实测（用户抓包验证）：
    ...
    async function getWebReportAnswers(paper, reportId, bizCode, token) {
        try {
            ...
            // v1.0.8fix：webreport 附带试题名称（data.title）与卷面题数（questions.length，复合题计 1 题），
    ...
    async function initReport(paperId, homeworkId, bizCode, platform, token) {
        ...
        // v1.0.8fix2：归属检测——若服务器忽略 homeworkId 参数并回显真实作业归属，说明锁卷不安全（部分试卷会 fallback）
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle, totalCount) {
        ...
        // v1.0.8fix：题数统计参考任务页面口径（paperInfo.questionCount 卷面题数，复合题计 1 题），
        ...
        results.forEach(r => {
            ...
            // v1.0.8fix：复合题子题分组显示（(1)(2)(3)…归属父题，不再拆成独立题目）
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        try {
            ...
            if (!answerReportId) {
                try {
                    // v1.0.8fix2：锁卷前归属检测——部分试卷服务器忽略 homeworkId='0' 并 fallback 真实作业，锁卷即生成真实作答报告污染成绩
            ...
            // v1.0.8fix：从 paperInfo 构建「复合题子题编号/题型」映射（questionId → questionNoShow/cateName，如 (1) 填空）
            ...
            // v1.0.8fix：优先走 webreport 全量答案（1 次 GET 拿全卷 rightAnswer/analyse/子题），
            ...
            for (const item of validAnswers) {
                ...
                if (childQs.length > 0 && parentRight.length === 0) {
                    // v1.0.8fix：复合题作为 1 题展示，子题 (1)(2)(3)…归属父题分组显示，不再拆成独立题目
                    ...
                    const subResults = childQs.map((child, childIdx) => {
                        ...
                        // v1.0.8fix：子题为多选/不定项时，选项并列显示 C、D（用 paperInfo 子题 cateName 判断）
                    ...
                    // v1.0.8fix：多选题/不定项选择题的多个选项是「并列答案」，应显示 C、D，
            ...
            // v1.0.8fix：题数优先级 paperInfo.questionCount（任务口径）→ webreport.questionCount（卷面题数）→ 展开条数
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 学习助手opt <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.8</span></div>
          ...
          <div class="eph-about-section">
            ...
            <div class="eph-about-rate-item"><span class="dot"></span> 使用本工具即代表同意合理使用</div>
                              ...
                              <div class="eph-about-section">
            <h3>✨ v1.0.8 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 锁卷安全机制升级：提交答案优先使用「任务已绑定报告」（原报告答题），锁卷报告仅作答案源，杜绝污染任务成绩</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 锁卷前归属检测：服务器忽略 homeworkId=0 的试卷自动放弃锁卷，不再生成真实作答报告</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 三层防线（创建回显检测 / 锁卷前归属检测 / 原报告答题），刷题成绩归属永远正确</div>
          </div>
<div class="eph-about-section">
            <h3>👨‍💻 作者团队</h3>
    ...
    async function loadPapers() {
        ...
        if (currentTask) {
            try {
                ...
                // v1.0.8fix：URL bizCode=201（VIEW）不可信 → 探测真实作答类型（204 习题优先，205 试卷兜底）
    ...
    async function brushPaper(paper) {
        ...
        try {
            ...
            // ① 选择答案源报告（已完成报告才支持取答案）
            // v1.0.8fix2：锁卷前记录「任务已绑定报告」（原报告答题）——
            ...
            // v1.0.8fix2：优先使用「任务已绑定报告」提交（原报告答题）——
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.8");
        loadPapers().catch(e => {
