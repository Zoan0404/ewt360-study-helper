...
(function() {
    ...
    const cleanHtmlKeepImg = (text) => {
        if (!text) return '';
        // v1.0.7fix：公式/图片 http → https（https 页面加载 http 混合内容会被浏览器拦截，导致公式图显示为空）
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle, totalCount) {
        ...
        // v1.0.7fix：题数统计参考任务页面口径（paperInfo.questionCount 卷面题数，复合题计 1 题），
        ...
        results.forEach(r => {
            ...
            // v1.0.7fix：复合题子题分组显示（(1)(2)(3)…归属父题，不再拆成独立题目）
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        try {
            ...
            // v1.0.7fix：从 paperInfo 构建「复合题子题编号/题型」映射（questionId → questionNoShow/cateName，如 (1) 填空）
            ...
            for (const item of validAnswers) {
                ...
                if (childQs.length > 0 && parentRight.length === 0) {
                    // v1.0.7fix：复合题作为 1 题展示，子题 (1)(2)(3)…归属父题分组显示，不再拆成独立题目
                    ...
                    const subResults = childQs.map((child, childIdx) => {
                        ...
                        // v1.0.7fix：子题为多选/不定项时，选项并列显示 C、D（用 paperInfo 子题 cateName 判断）
                    ...
                    // v1.0.7fix：多选题/不定项选择题的多个选项是「并列答案」，应显示 C、D，
            ...
            // v1.0.7fix：题数优先级 paperInfo.questionCount（任务口径）→ webreport.questionCount（卷面题数）→ 展开条数
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 学习助手opt <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.7</span></div>
