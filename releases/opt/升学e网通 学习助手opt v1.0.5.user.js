...
// @description  采用志成🍥 的答案获取与风月同天🌸 的脚本参考及UI页面,由⚡Zoan进行整合并支持子母题型 混合题型 并且提供扫描功能,支持检测完成状态,优化提交功能,支持查看答案,进一步优化速度。v1.0.4：集成官方「重新作答」接口 reAnswerPaper（204课后习题重刷后任务入口自动绑定最新报告，实测有效）；205试卷服务器拒绝重做（7771522），任务绑定固化无法改写，强制重刷仅生成满分报告并提示；锁卷取答案（刷题与查看答案）改用专用 homeworkId=0 报告，杜绝污染任务成绩与主报告；新增速度档位设置（快/中/稳）与强制重刷开关。v1.0.5：答案弹窗显示修复——题数按卷面统计（复合题计1题）、复合题子题(1)(2)(3)归属父题分组显示并标注题型、多选/不定项答案并列显示；公式图片http→https升级；浏览页(bizCode=201)自动探测真实作答类型(204/205)，失败回退201。
// @license      ISC
...
(function() {
    ...
    function getCurrentPageTask() {
        ...
        // v1.0.5fix：URL 上的 bizCode=201（VIEW 浏览码）不是真实作答类型，
    ...
    const cleanHtmlKeepImg = (text) => {
        if (!text) return '';
        // v1.0.5fix：公式/图片 http → https（https 页面加载 http 混合内容会被浏览器拦截，导致公式图显示为空）
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle, totalCount) {
        ...
        // v1.0.5fix：题数统计参考任务页面口径（paperInfo.questionCount 卷面题数，复合题计 1 题），
        ...
        results.forEach(r => {
            ...
            // v1.0.5fix：复合题子题分组显示（(1)(2)(3)…归属父题，不再拆成独立题目）
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        try {
            ...
            // v1.0.5fix：从 paperInfo 构建「复合题子题编号/题型」映射（questionId → questionNoShow/cateName，如 (1) 填空）
            ...
            for (const item of validAnswers) {
                ...
                if (childQs.length > 0 && parentRight.length === 0) {
                    // v1.0.5fix：复合题作为 1 题展示，子题 (1)(2)(3)…归属父题分组显示，不再拆成独立题目
                    ...
                    const subResults = childQs.map((child, childIdx) => {
                        ...
                        // v1.0.5fix：子题为多选/不定项时，选项并列显示 C、D（用 paperInfo 子题 cateName 判断）
                    ...
                    // v1.0.5fix：多选题/不定项选择题的多个选项是「并列答案」，应显示 C、D，
            ...
            showAnswerModal(expandedResults, expandedQuestions, paper.paperId, '1', paper.bizCode, paper.homeworkId, paper.title, paperInfo?.questionCount);
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 学习助手opt <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.5</span></div>
    ...
    async function loadPapers() {
        ...
        if (currentTask) {
            try {
                ...
                // v1.0.5fix：URL bizCode=201（VIEW）不可信 → 探测真实作答类型（204 习题优先，205 试卷兜底）
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.5");
        loadPapers().catch(e => {
