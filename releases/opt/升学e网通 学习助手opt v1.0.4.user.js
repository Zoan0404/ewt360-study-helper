...
// @description  采用志成🍥 的答案获取与风月同天🌸 的脚本参考及UI页面,由⚡Zoan进行整合并支持子母题型 混合题型 并且提供扫描功能,支持检测完成状态,优化提交功能,支持查看答案,进一步优化速度。v1.0.4：集成官方「重新作答」接口 reAnswerPaper（204课后习题重刷后任务入口自动绑定最新报告，实测有效）；205试卷服务器拒绝重做（7771522），任务绑定固化无法改写，强制重刷仅生成满分报告并提示；锁卷取答案（刷题与查看答案）改用专用 homeworkId=0 报告，杜绝污染任务成绩与主报告；新增速度档位设置（快/中/稳）与强制重刷开关。
// @license      ISC
...
(function() {
    ...
    // ==================== 仅获取答案功能 ====================
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle) {
        ...
        title.textContent = `📝 ${displayTitle} 答案 (共 ${results.length} 题)`;
        ...
        results.forEach(r => {
            ...
            qDiv.innerHTML = `
                ...
                <div style="color:#e53e3e; margin:4px 0 6px; font-weight:500;">答案: ${ansHtml}</div>
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        try {
            ...
            for (const item of validAnswers) {
                ...
                if (childQs.length > 0 && parentRight.length === 0) {
                    ...
                    childQs.forEach((child) => {
                        ...
                        const newQ = {
                            questionId: childId, questionNumber: '', cateId: cate,
                            subjective: subjective, groupName: groupName,
                        ...
                        expandedResults.push({
                            num: '', group: groupName, answer: answerStr,
                            knowledge: childKnowledge, analysis: cleanHtmlKeepImg(childAnalyse),
                            images: childImages, rawRightAnswer: childRight,
                            questionId: childId, subjective: subjective,
                        });
                    });
                } else {
            ...
            showAnswerModal(expandedResults, expandedQuestions, paper.paperId, '1', paper.bizCode, paper.homeworkId, paper.title);
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 学习助手opt <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.4</span></div>
          ...
          <div class="eph-about-section">
            <h3>✨ v1.0.4 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 集成官方「重新作答」接口：204课后习题重刷后入口自动绑定最新报告</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 205试卷任务成绩服务器固化，无法改写（重刷仅生成满分报告并提示）</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 刷题与查看答案的锁卷取答案改用 homeworkId=0 专用报告，不会污染任务成绩与主报告</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 新增速度档位设置（🟢快 / 🟡中 / 🐢稳）与「强制重刷」开关</div>
          </div>
          <div class="eph-about-section">
            <h3>👨‍💻 作者团队</h3>
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.4");
        loadPapers().catch(e => {
