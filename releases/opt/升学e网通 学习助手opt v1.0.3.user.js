...
(function() {
    ...
    const RETRY_COUNT = 2;
    const CONCURRENCY = { BRUSH: Infinity, ANSWER: 10, SCAN: 3 };   // 优化：BRUSH 一次全部刷完
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        const token = getToken();
        try {
            // 使用 paper.bizCode 获取报告ID，而不是硬编码 BIZ.VIEW
            const reportId = await getReportId(paper.paperId, paper.bizCode, token);
            const paperInfo = await getPaperInfo(paper.paperId, reportId, '1', paper.bizCode, token);
            ...
            // 更新报告也使用 paper.bizCode
            await updateReport(paper.paperId, reportId, paper.bizCode, '1', token);
            const questions = await getQuestions(paper.paperId, reportId, '1', paper.bizCode, token);
            // 并发获取答案
            const allAnswers = await mapLimit(questions, CONCURRENCY.ANSWER, async (q) => {
                const ans = await getAnswer(paper.paperId, reportId, q.questionId, paper.bizCode, '1', token);
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 全能刷课助手</div>
    ...
    async function brushPaper(paper) {
        ...
        const token = getToken();
        try {
            // 获取信息报告ID，使用 paper.bizCode 而不是固定 BIZ.VIEW
            const infoReportId = await getReportId(paper.paperId, paper.bizCode, token);
            // 提交报告ID，使用 paper.bizCode 创建
            const submitReportId = await initReport(paper.paperId, paper.homeworkId, paper.bizCode, '1', token);

            // 获取试卷信息，使用 infoReportId 和 paper.bizCode
            const paperInfo = await getPaperInfo(paper.paperId, infoReportId, '1', paper.bizCode, token);
            const fullScoreMap = new Map();
            const metaMap = new Map();
            const traverseQuestions = (list) => {
                for (const q of list) {
            ...
            // 更新报告（使用 paper.bizCode）
            await updateReport(paper.paperId, infoReportId, paper.bizCode, '1', token);

            const questions = await getQuestions(paper.paperId, infoReportId, '1', paper.bizCode, token);

            // 并发获取每道题的答案
            const allAnswers = await mapLimit(questions, CONCURRENCY.ANSWER, async (q) => {
                const ans = await getAnswer(paper.paperId, infoReportId, q.questionId, paper.bizCode, '1', token);
                return ans ? { question: q, answer: ans } : null;
            });
            const validAnswers = allAnswers.filter(Boolean);
