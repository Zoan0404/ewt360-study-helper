...
(function() {
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle) {
        ...
        results.forEach(r => {
            ...
            `;
            let ansHtml = r.answer.replace(/&lt;/g, "<").replace(/&gt;/g, ">");
            let parseHtml = r.analysis ? r.analysis.replace(/\\</g, "<").replace(/\\:/g, ":") : '';
            qDiv.innerHTML = `
                <div style="font-weight:700; color:#2d3748; margin-bottom:4px; font-size:15px;">[${r.num}] ${r.group}</div>
                <div style="color:#e53e3e; margin:4px 0 6px; font-weight:500;">答案: ${ansHtml}</div>
                ${r.knowledge ? `<div style="color:#718096; font-size:12px; margin:2px 0;">🧠 知识点: ${r.knowledge}</div>` : ''}
                ${r.analysis ? `<div style="color:#4a5568; font-size:13px; white-space:pre-wrap; margin-top:6px; padding:8px 12px; background:#ffffff; border-radius:8px;">📖 解析: ${parseHtml}</div>` : ''}
            `;
