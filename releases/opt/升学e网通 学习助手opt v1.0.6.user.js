...
// @description  采用志成🍥 的答案获取与风月同天🌸 的脚本参考及UI页面,由⚡Zoan进行整合并支持子母题型 混合题型 并且提供完整扫描功能,支持检测完成状态,优化提交功能,支持查看答案,进一步优化速度.
// @license      ISC
...
(function() {
    ...
    function getCurrentPageTask() {
        ...
        // v1.0.6fix：URL 上的 bizCode=201（VIEW 浏览码）不是真实作答类型，
    ...
    const cleanHtmlKeepImg = (text) => {
        if (!text) return '';
        // v1.0.6fix：公式/图片 http → https（https 页面加载 http 混合内容会被浏览器拦截，导致公式图显示为空）
    ...
    async function getReportId(paperId, bizCode, token) {
        ...
        const data = await gmFetch("GET", url, { token, 'User-Agent': UA });
    ...
    async function getUserId() {
        ...
        const data = await gmFetch("GET", url, { token: getToken(), 'User-Agent': UA });
    ...
    async function getQuestions(paperId, reportId, platform, bizCode, token) {
        ...
        const data = await gmFetch("POST", url, { token, 'User-Agent': UA }, body);
        const questions = [];
        if (!data || !Array.isArray(data.groupQuestionList)) {
            ...
            const data2 = await gmFetch("POST", url2, { token, 'User-Agent': UA }, body2);
    ...
    async function updateReport(paperId, reportId, bizCode, platform, token) {
        ...
        await gmFetch("POST", url, { token, 'User-Agent': UA }, body);
    }
    ...
    async function reAnswerPaper(reportId, token) {
        ...
        const data = await gmFetch("POST", url, { token, 'User-Agent': UA }, { reportId });
    ...
    async function getAnswer(paperId, reportId, questionId, bizCode, platform, token) {
        ...
        try {
            return await gmFetch("POST", url, { token, 'User-Agent': UA }, body);
        } catch (e) {
    ...
    async function getPaperInfo(paperId, reportId, platform, bizCode, token) {
        ...
        return await gmFetch("GET", url, { token, 'User-Agent': UA });
    }

    // ---------- v1.0.6fix：webreport 全量答案接口（1 次 GET 拿全卷答案+解析，替代逐题 analysis） ----------
    // 实测（用户抓包验证）：
    ...
    async function getWebReportAnswers(paper, reportId, bizCode, token) {
        try {
            ...
            if (bizCode === BIZ.PAPER) {
                ...
                raw = await gmFetch("GET", url, { token, 'User-Agent': UA });
                const questions = [];
            ...
            // v1.0.6fix：webreport 附带试题名称（data.title）与卷面题数（questions.length，复合题计 1 题），
    ...
    async function initReport(paperId, homeworkId, bizCode, platform, token) {
        ...
        let data;
        try {
            data = await gmFetch("GET", url, { token, 'User-Agent': UA });
        } catch (e) {
            if (e.message && e.message.includes('已做过')) {
                const retryUrl = url.replace('isRepeat=0', 'isRepeat=1');
                data = await gmFetch("GET", retryUrl, { token, 'User-Agent': UA });
            } else {
                throw e;
            }
        }
        if (!data || !data.reportId) throw new Error('初始化报告失败: reportId为空');
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle, totalCount) {
        ...
        // v1.0.6fix：题数统计参考任务页面口径（paperInfo.questionCount 卷面题数，复合题计 1 题），
        ...
        results.forEach(r => {
            ...
            // v1.0.6fix：复合题子题分组显示（(1)(2)(3)…归属父题，不再拆成独立题目）
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        try {
            ...
            // v1.0.6fix：从 paperInfo 构建「复合题子题编号/题型」映射（questionId → questionNoShow/cateName，如 (1) 填空）
            ...
            // v1.0.6fix：优先走 webreport 全量答案（1 次 GET 拿全卷 rightAnswer/analyse/子题），
            ...
            for (const item of validAnswers) {
                ...
                if (childQs.length > 0 && parentRight.length === 0) {
                    // v1.0.6fix：复合题作为 1 题展示，子题 (1)(2)(3)…归属父题分组显示，不再拆成独立题目
                    ...
                    const subResults = childQs.map((child, childIdx) => {
                        ...
                        // v1.0.6fix：子题为多选/不定项时，选项并列显示 C、D（用 paperInfo 子题 cateName 判断）
                    ...
                    // v1.0.6fix：多选题/不定项选择题的多个选项是「并列答案」，应显示 C、D，
            ...
            // v1.0.6fix：题数优先级 paperInfo.questionCount（任务口径）→ webreport.questionCount（卷面题数）→ 展开条数
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 学习助手opt <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.6</span></div>
          ...
          <div class="eph-about-section">
                      <div class="eph-about-section">
            <h3>✨ v1.0.6 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 新增 webreport 报告接口全量答案：1 次请求获取全卷答案+解析（替代逐题并发），速度大幅提升</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 复合题子题答案/解析随全量返回，分组显示更完整（含子题题型标注）</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 附带试题名称与卷面题数，弹窗题数三级兜底（任务口径→webreport→展开条数）</div>
          </div>
<h3>✨ v1.0.5 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 答案弹窗题数修正：按卷面题目数统计（复合题计1题），不再把子题拆开计数</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 复合题子题 (1)(2)(3)…归属父题分组显示，并标注子题题型（填空/单选）</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 多选/不定项答案并列显示（C、D），不再误用多空格式（1、C;2、D;）</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 浏览页(bizCode=201)自动探测真实作答类型（204习题/205试卷），失败回退201</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 公式图片 http→https 升级，修复 https 页面公式图不显示</div>
          </div>
          <div class="eph-about-section">
            <h3>👨‍💻 作者团队</h3>
    ...
    async function loadPapers() {
        ...
        if (currentTask) {
            try {
                ...
                // v1.0.6fix：URL bizCode=201（VIEW）不可信 → 探测真实作答类型（204 习题优先，205 试卷兜底）
    ...
    async function brushPaper(paper) {
        ...
        try {
            ...
            // v1.0.4：已完成报告无需锁卷（finish=true 直接可取答案）；
            // 锁卷兜底报告已在上面完成锁卷，此处不再重复锁卷
            const questions = await getQuestions(paper.paperId, answerReportId, '1', paper.bizCode, token);
            ...
            // 并发获取每道题的答案
            const allAnswers = await mapLimit(questions, CONCURRENCY.ANSWER, async (q) => {
                const ans = await getAnswer(paper.paperId, answerReportId, q.questionId, paper.bizCode, '1', token);
                return ans ? { question: q, answer: ans } : null;
            });
            const validAnswers = allAnswers.filter(Boolean);
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.6");
        loadPapers().catch(e => {
