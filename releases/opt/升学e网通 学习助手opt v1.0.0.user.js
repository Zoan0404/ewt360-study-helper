...
// @description  采用志成🍥 的答案获取与风月同天🌸 的脚本参考及UI页面,由⚡Zoan进行整合并支持子母题型 混合题型 并且提供扫描功能,支持检测完成状态,优化提交功能,支持查看答案
// @license      ISC
...
(function() {
    ...
    const RETRY_COUNT = 2;
    const CONCURRENCY = { BRUSH: 2, ANSWER: 5, SCAN: 3 };
    ...
    async function submitAllAndCorrect(paperId, platform, bizCode, questions, results, homeworkId, token) {
        const reportId = await initReport(paperId, homeworkId, bizCode, platform, token);
        ...
        await submitAnswersBatch(paperId, reportId, homeworkId, answers, bizCode, platform, token);
        await submitPaper(paperId, reportId, homeworkId, bizCode, platform, token);
        await submitCorrected(paperId, reportId, bizCode, platform, token);
    }
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle) {
        ...
        body.style.cssText = `
            ...
            line-height: 1.7; font-size: 14px; color: #2d3748;
            background: #ffffff;
        `;
        results.forEach(r => {
    ...
    async function fetchAndShowAnswers(paper) {
        ...
        const token = getToken();
        try {
            const reportId = await getReportId(paper.paperId, BIZ.VIEW, token);
            const paperInfo = await getPaperInfo(paper.paperId, reportId, '1', BIZ.VIEW, token);
            ...
            await updateReport(paper.paperId, reportId, BIZ.VIEW, '1', token);
            const questions = await getQuestions(paper.paperId, reportId, '1', BIZ.VIEW, token);
            const allAnswers = [];
            for (const q of questions) {
                const ans = await getAnswer(paper.paperId, reportId, q.questionId, BIZ.VIEW, '1', token);
                if (ans) {
                    allAnswers.push({ question: q, answer: ans });
                }
            }
...
`;
    ...
    function renderAnswerView(container, groups) {
        ...
        for (const group of groups) {
            ...
            for (const paper of group.papers) {
                ...
                info.innerHTML = `<div class="eph-paper-title">${typeLabel}${escapeHtml(paper.title)}</div>
    ...
    async function brushPaper(paper) {
        ...
        console.log(`paperId=${paper.paperId}, bizCode=${paper.bizCode}, homeworkId=${paper.homeworkId}`);
        showProgress(panel.progress, `正在刷: ${paper.title}`, "获取答案中...");
        ...
        try {
            ...
            await submitAllAndCorrect(paper.paperId, '1', paper.bizCode, expandedQuestions, expandedResults, paper.homeworkId, token);
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.0");
        loadPapers().catch(e => {
