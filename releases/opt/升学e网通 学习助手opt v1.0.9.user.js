...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/591258/%E5%8D%87%E5%AD%A6e%E7%BD%91%E9%80%9A%20%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8Bopt.user.js
// @updateURL https://update.greasyfork.org/scripts/591258/%E5%8D%87%E5%AD%A6e%E7%BD%91%E9%80%9A%20%E5%AD%A6%E4%B9%A0%E5%8A%A9%E6%89%8Bopt.meta.js
// ==/UserScript==
...
(function() {
    ...
    function getCurrentPageTask() {
        ...
        // v1.0.9fix：URL 上的 bizCode=201（VIEW 浏览码）不是真实作答类型，
    ...
    const cleanHtmlKeepImg = (text) => {
        if (!text) return '';
        // v1.0.9fix：公式/图片 http → https（https 页面加载 http 混合内容会被浏览器拦截，导致公式图显示为空）
    ...
    // v1.0.9：答案多字段兜底提取（借鉴 ewttest extractAnswerText：rightAnswer/rightAnswers/answers/answer/standardAnswer/correctAnswer/answerContent/trueAnswer/myAnswer/answerList）
    ...
    // v1.0.9：解析多字段兜底提取（借鉴 ewttest extractAnalysisHtml：analyse/analysis/analysisContent/analyseContent/answerAnalysis/parse/parseContent/explanation）
    ...
    // v1.0.9：附件图片统一 http → https（借鉴 ewttest normalizeImg）
    ...
    async function getReportId(paperId, bizCode, token) {
        ...
        // v1.0.9：报告ID多字段提取（reportId || report || id，借鉴 ewttest）
    ...
    // v1.0.9fix：模块级缓存——刷一套卷 userId 只请求 1 次（webreport / getQuestions 降级共用），不再重复 3-5 次
    ...
    async function getAnswer(paperId, reportId, questionId, bizCode, platform, token) {
        ...
        try {
            ...
            if (ans) {
                // v1.0.9：多字段兜底归一化（借鉴 ewttest）——rightAnswer/analyse 缺失时从其他字段补
    ...
    // ---------- v1.0.9fix：webreport 全量答案接口（1 次 GET 拿全卷答案+解析，替代逐题 analysis） ----------
    // 实测（用户抓包验证）：
    ...
    async function getWebReportAnswers(paper, reportId, bizCode, token) {
        try {
            ...
            // v1.0.9fix：webreport 附带试题名称（data.title）与卷面题数（questions.length，复合题计 1 题），
    ...
    async function initReport(paperId, homeworkId, bizCode, platform, token) {
        ...
        // v1.0.9fix2：归属检测——若服务器忽略 homeworkId 参数并回显真实作业归属，说明锁卷不安全（部分试卷会 fallback）
    ...
    function showAnswerModal(results, questions, paperId, platform, submitBizCode, homeworkId, paperTitle, totalCount) {
        ...
        // v1.0.9fix：题数统计参考任务页面口径（paperInfo.questionCount 卷面题数，复合题计 1 题），
        ...
        results.forEach(r => {
            ...
            // v1.0.9fix：复合题子题分组显示（(1)(2)(3)…归属父题，不再拆成独立题目）
    ...
    // v1.0.4 修复：答案源选择与锁卷兜底（不再直接锁主报告，杜绝污染任务成绩）
    // 实测：analysis 仅在报告 finish=true 时返回正确答案；锁卷专用报告(homeworkId='0')不参与任务绑定
    async function fetchAndShowAnswers(paper) {
        ...
        try {
            ...
            // v1.0.9：末选：bizCode=201 查看态报告（借鉴 ewttest 5 候选兜底，analysis 兼容 201）
            ...
            if (!answerReportId) {
                try {
                    // v1.0.9fix2：锁卷前归属检测——部分试卷服务器忽略 homeworkId='0' 并 fallback 真实作业，锁卷即生成真实作答报告污染成绩
                    // 通过报告详情接口检查归属：服务器回显 homeworkId≠'0' → 已绑定真实作业 → 放弃锁卷（未完成空报告不产生成绩，无污染）
                    ...
                    if (lockSt && lockSt.homeworkId !== undefined && String(lockSt.homeworkId) !== '0') {
                        console.warn(`[锁卷] 该试卷 homeworkId=0 无效：新报告 ${lockId} 归属 homeworkId=${lockSt.homeworkId}（已绑定真实作业），放弃锁卷避免污染成绩`);
                        throw new Error('锁卷报告被绑定真实作业，放弃（不污染成绩）');
                    }
                    await updateReport(paper.paperId, lockId, paper.bizCode, '1', token);
                    answerReportId = lockId;
                    console.log(`[获取答案] 无已完成报告，新建锁卷报告 ${lockId}(homeworkId=0) 换取答案（兜底，不污染任务成绩）`);
                } catch (e) {
                    console.warn('[获取答案] 锁卷兜底失败:', e.message);
                }
            }

            // v1.0.4：答案源报告必须存在（无报告则无法取答案）
            if (!answerReportId) {
            ...
            // v1.0.9fix：从 paperInfo 构建「复合题子题编号/题型」映射（questionId → questionNoShow/cateName，如 (1) 填空）
            ...
            // v1.0.4：已完成报告无需锁卷（finish=true 直接可取答案）；
            // 锁卷兜底报告已在上面完成锁卷，此处不再重复锁卷
            // v1.0.9fix：优先走 webreport 全量答案（1 次 GET 拿全卷 rightAnswer/analyse/子题），
            ...
            for (const item of validAnswers) {
                ...
                if (childQs.length > 0 && parentRight.length === 0) {
                    // v1.0.9fix：复合题作为 1 题展示，子题 (1)(2)(3)…归属父题分组显示，不再拆成独立题目
                    ...
                    const subResults = childQs.map((child, childIdx) => {
                        ...
                        // v1.0.9fix：子题为多选/不定项时，选项并列显示 C、D（用 paperInfo 子题 cateName 判断）
                    ...
                    // v1.0.9fix：多选题/不定项选择题的多个选项是「并列答案」，应显示 C、D，
            ...
            // v1.0.9fix：题数优先级 paperInfo.questionCount（任务口径）→ webreport.questionCount（卷面题数）→ 展开条数
...
`;
    ...
    function createPanel(cbs) {
        ...
        aboutContent.innerHTML = `
          ...
          <div class="eph-about-name">升学e网通 学习助手opt <span style="font-size:12px;color:#a78bfa;font-weight:600;">v1.0.9</span></div>
          ...
          <div class="eph-about-section">
            ...
            <div class="eph-about-rate-item"><span class="dot"></span> 使用本工具即代表同意合理使用</div>
                              ...
                              <div class="eph-about-section">
            <h3>✨ v1.0.9 更新内容</h3>
            <div class="eph-about-rate-item"><span class="dot"></span> 答案获取增强：答案/解析多字段兜底提取（9 种答案字段 + 8 种解析字段），服务器字段变化不再丢答案</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 附件图片统一 http→https，公式图/插图显示更稳定</div>
            <div class="eph-about-rate-item"><span class="dot"></span> 报告ID多字段提取（reportId/report/id）+ 答案源增加 201 查看态候选，取答案成功率更高</div>
          </div>
<div class="eph-about-section">
            <h3>👨‍💻 作者团队</h3>
          ...
          <div class="eph-about-section">
            ...
            <div>
              ...
              <div class="eph-about-link-item"><span>📦 源码仓库</span><a href="https://github.com/Zxxaq1478359473/ewt360-study-helper" target="_blank">GitHub</a></div>
    ...
    function renderPaperList(container, groups, onBrush) {
        ...
        for (const group of groups) {
            ...
            for (const paper of group.papers) {
                ...
                // v1.0.4：默认已完成禁用；开启「强制重刷」后显示「重做」并允许点击（204习题走官方 reAnswerPaper，绑定自动更新）
    ...
    let forceRedo = false;   // v1.0.4：强制重刷开关（204习题走官方 reAnswerPaper 重做；205试卷仅生成满分报告，任务成绩固化）
    ...
    async function brushAll() {
        ...
        for (const group of groupsCache) {
            for (const paper of group.papers) {
                // v1.0.4：默认跳过已完成；开启「强制重刷」后所有任务都刷（204习题走官方 reAnswerPaper 重做）
    ...
    async function loadPapers() {
        ...
        if (currentTask) {
            try {
                ...
                // v1.0.9fix：URL bizCode=201（VIEW）不可信 → 探测真实作答类型（204 习题优先，205 试卷兜底）
    ...
    async function brushPaper(paper) {
        ...
        try {
            ...
            // ① 选择答案源报告（已完成报告才支持取答案）
            // v1.0.9fix2：锁卷前记录「任务已绑定报告」（原报告答题）——
            ...
            // v1.0.9fix2：优先使用「任务已绑定报告」提交（原报告答题）——
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.9");
        loadPapers().catch(e => {
