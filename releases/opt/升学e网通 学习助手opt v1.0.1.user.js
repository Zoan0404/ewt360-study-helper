...
`;
    ...
    async function brushPaper(paper) {
        ...
        try {
            ...
            const [viewReportId, submitReportId] = await Promise.all([
                getReportId(paper.paperId, BIZ.VIEW, token),
                initReport(paper.paperId, paper.homeworkId, paper.bizCode, '1', token)
            ]);

            // 获取试卷信息
            const paperInfo = await getPaperInfo(paper.paperId, viewReportId, '1', BIZ.VIEW, token);
            const fullScoreMap = new Map();
            const metaMap = new Map();
            const traverseQuestions = (list) => {
                for (const q of list) {
            ...
            await updateReport(paper.paperId, viewReportId, BIZ.VIEW, '1', token);

            const questions = await getQuestions(paper.paperId, viewReportId, '1', BIZ.VIEW, token);

            // 优化：并发获取每道题的答案
            const allAnswers = await mapLimit(questions, CONCURRENCY.ANSWER, async (q) => {
                const ans = await getAnswer(paper.paperId, viewReportId, q.questionId, BIZ.VIEW, '1', token);
                return ans ? { question: q, answer: ans } : null;
            });
            const validAnswers = allAnswers.filter(Boolean);
    ...
    if (!getToken()) {
        ...
        panel.show();
        console.log("[EWT Helper] 已启动，版本 v1.0.1");
        loadPapers().catch(e => {
